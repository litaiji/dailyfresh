...
class LoginView(View):
    ...
    def get(self, request):
        ...
        if 'username' in request.COOKIES:
            ...
            username = ''
            checked = ''
        return render(request, 'login.html',{'username':username, 'checked':checked}) # **locals()
    ...
    def post(self, request):
        ...
        if user is not None:
            ...
            if user.is_active:
                ...
                # cache.set('username', username, 7*24*3600)
                # 获取登录后所要跳转到的地址
                # 默认跳转到首页
                # 判断是否需要记住用户名
...
class UserInfoView(LoginRequiredMixin, View):
    ...
    def get(self, request):
        ...
        # sr = StrictRedis(host='10.0.128.126', port='6379', db=9)
        ...
        goods_li = []
        for id in sku_ids:
            goods = GoodsSKU.objects.fiter(id=id)
        ...
        # 除了你给模板文件传递模板变量之外，Django框架会把request.user也传给模板文件
